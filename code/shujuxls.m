function [X0,Y0,H,mx,my,h,caiyang,test,chazhi]=shujuxls(caiyangshuju,testshuju,chuzhishuju);%%%%%%%%读取ARCGIS倒出来的TXT栅格文件和XLS格式的采样数据
...
if length(caiyangshuju)>0
    ...
    caiyang(caiyang(:,2)<(h),:)=[];
    caiyang(caiyang(:,2)>((my-2)*h),:)=[]; %%%把超出范围的采样点去掉
    x=caiyang(:,1);
    y=caiyang(:,2);
